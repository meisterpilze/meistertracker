...
describe('sendcloud adapter', () => {
  ...
  it('#10 rejects an unknown country instead of buying an invalid label', async () => {
  ...
  it('#10 refuses a non-EU destination (EU-only — no UK, no Switzerland)', async () => {
    const restore = mockFetch(async () => jsonRes(200, { parcel: { id: 1, label: {} } }));
    try {
      for (const c of ['Switzerland', 'CH', 'United Kingdom', 'GB', 'England', 'US', 'Norway']) {
        await assert.rejects(
          () =>
            ship.sendcloud.buyLabel(cfg, {
              order: { id: 1, shipCountry: c },
              methodId: 1,
              weightG: 1000,
              requestLabel: false
            }),
