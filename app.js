  ...
  if(body){
    days.forEach((d,i)=>{
      ...
      timed.forEach(e=>{
        ...
        el.dataset.date=ds;
        el.onclick=function(ev){if(!el._dragged)onCalEventClick(e)};
        if(e.type==='custom'){
...
function snapPx(px){return Math.round(px/12)*12}// snap to 15-min increments (48px/4=12)
...
function deleteCalEventFromDetail(id){
  ...
  confirm2('Event löschen?','Dieses Event wird unwiderruflich gelöscht.','Löschen',()=>{
    ...
    deleteCalendarEventAPI(id);renderCalendar();
  });
}
...
function deleteTaskFromCalendar(taskId){
  ...
  confirm2('Aufgabe löschen?','Diese Aufgabe wird unwiderruflich gelöscht.','Löschen',()=>{
    ...
    markDirty();renderManualTasks();renderCalendar();updateTodoBadge();
  });
}
...
function importBackup(){const file=document.getElementById('import-file').files[0];if(!file)return;confirm2('Restore this backup?','Replaces ALL data on the server for all users. Cannot be undone.','Yes, restore',()=>{const r=new FileReader();r.onload=async e=>{try{const d=JSON.parse(e.target.result);const prev={batches,scanLog,manualTasks,harvests,cultures,inventory,assets};batches=d.batches||[];scanLog=d.scanLog||[];manualTasks=d.manualTasks||[];harvests=d.harvests||[];cultures=d.cultures||[];inventory=d.inventory||defaultInventory();assets=d.assets||[];batches.forEach(b=>spColor(b.species));const resp=await authFetch('/api/backup/restore',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({batches,scanLog,manualTasks,harvests,cultures,inventory,assets,caldav,teamMembers})});const res=await resp.json().catch(()=>({}));if(!resp.ok||!res.ok){batches=prev.batches;scanLog=prev.scanLog;manualTasks=prev.manualTasks;harvests=prev.harvests;cultures=prev.cultures;inventory=prev.inventory;assets=prev.assets;alert('Restore failed – server rejected the request. Please check your login and try again.');return;}alert('Restored successfully.');go('dash','n-dash');}catch(err){if(err.message==='unauthorized')return;alert('Failed to restore: '+(err.message||err))}};r.readAsText(file)})}
