...
function undoScanEntry(idx){
  ...
  confirm2('Scan rückgängig machen?','Diesen Scan-Eintrag wirklich rückgängig machen?\n\n'+desc,'Ja, rückgängig',()=>{
    ...
    saveData();updateSD();renderSuccessList();
...
function processScan(raw){
  ...
  if(batch||isBag){
    ...
    const entry={time:new Date().toISOString(),action:scan.action,batch:batchId,bag:isBag?val:null,from:scan.from,to:scan.to,species:batch?.species,strain:batch?.strain};scanLog.push(entry);_sessionSuccesses.push(entry);scan.count++;saveData();
