...
If the project lives under a OneDrive-synced folder (`OneDrive - Meisterpilze UG`), the `backups/` directory cloud-syncs automatically. This is convenient but **platform-specific and not portable** — on a Linux server you must use rsync (or another off-site tool that touches the marker file).
