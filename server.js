...
function ensureAcmeSh(callback) {
  ...
  execFile(
    BASH,
    ['-c', "curl -sSL https://get.acme.sh | sh -s -- --install-online --home '" + homeArg + "' --no-cron"],
    { timeout: 60000 },
    (err, stdout, stderr) => {
      if (err || !fs.existsSync(ACME_SH)) {
        log('error', 'acme.sh auto-install failed', { error: err ? err.message : 'binary not found', stderr });
        return callback(
          new Error('acme.sh installation failed — ensure curl and bash (Git for Windows) are available')
        );
      }
...
function requestLetsEncryptCert(callback) {
  ...
  ensureAcmeSh((installErr) => {
    ...
    runAcmeSh(
      ['--issue', '--dns', 'dns_duckdns', '-d', fullDomain, '--home', home, '--server', 'letsencrypt', '--force'],
      env,
      120000,
      (err, stdout, stderr) => {
        if (err) {
...
function _doAutoSyncAllCaldav(data) {
  try {
    ...
    try {
      ...
      log('warn', 'CalDAV: failed to clean orphaned files', { error: e.message });
    }
  } catch (e) {
  ...
  if (parts.length === 1 && parts[0] === 'calendars') {
    ...
    if (depth !== '0') {
      for (const cal of cals) {
        const displayName =
          cal === 'meisterpilze'
            ? 'Meisterpilze (Betrieb)'
            : cal.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
...
<d:multistatus xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav" xmlns:cs="http://calendarserver.org/ns/">
  ...
  if (parts.length === 2 && parts[0] === 'calendars') {
    ...
    const displayName =
      calName === 'meisterpilze'
        ? 'Meisterpilze (Betrieb)'
        : calName.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());
...
function handleRequest(req, res) {
  ...
  if (req.method === 'DELETE' && calEvMatch) {
    ...
    try {
      ...
      try {
        autoDeleteCalendarEventCaldav(id);
      } catch (ce) {
